...
north.onAdd = function(map) {
        ...
        // Menampilkan Data Titik Dengan Single Marker
        $.getJSON("<?=base_url()?>assets/provinsi.geojson",function(data){ 
         ...
         // Menampilkan Data titik dengan Multi Marker
        $.getJSON("<?=base_url()?>assets/gunung.geojson",function(data){ 
            var ratIcon = L.icon({
                iconUrl: '<?=base_url()?>assets/rsud.png', 
                iconSize: [12,10]
            });
            L.geoJson(data,{
                pointToLayer: function(feature,latlng){
        ...
        $.getJSON("<?=base_url()?>/assets/sungai.geojson",function(kode){ 
            L.geoJson( kode, { 
                style: function(feature){ 
                    var color,kode = feature.properties.kode; if ( kode < 2 ) color = "#f2051d";
                    else if ( kode > 0 ) color = "#f2051d"; else color = "#f2051d"; 
                    // no data
                    return { color: "#999", weight: 3, color: color, fillOpacity: .5 };
                },
        ...
        // Menampilkan Data Poligon 
        $.getJSON("<?=base_url()?>/assets/provinsi_polygon.geojson",function(kode){ L.geoJson( kode, {
            style: function(feature){ var fillColor,
                ...
                return { color: "#999", weight: 1, fillColor: fillColor, fillOpacity: .6 };
                 ...
                 onEachFeature: function( feature, layer ){ layer.bindPopup(feature.properties.PROV)
                }
            }).addTo(provin);
        });
